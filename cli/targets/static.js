...
function exportName(object, asInterface) {
  ...
  if (asInterface) parts[i - 1] = removeTypeNameSuffix(parts[i - 1]);
  ...
  if (config.comments) {
    var typeDef = [
      ...
      type.parent instanceof protobuf.Root
        ? "@exports " + escapeName(removeTypeNameSuffix(type.name))
    ...
    type.fieldsArray.forEach(function (field) {
      ...
      if (isOptional) jsType = jsType + "|null";
      typeDef.push(
        "@property {" +
          jsType +
          "} " +
          (isOptional ? "[" + prop + "]" : prop) +
          " " +
...
function buildEnum(ref, enm) {
  ...
  else push(escapeName(ref) + "." + escapeName(enm.name) + " = (function() {");
  ++indent;
  push(
    (config.es6 ? "const" : "var") +
